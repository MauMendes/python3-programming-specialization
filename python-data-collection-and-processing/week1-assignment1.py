...
#2) Below, a list of lists is provided. Use in and not in tests to create variables with Boolean values. See comments for 
...
twentyfour = False
for items in nested['data']:
    if 24 is items:
        twentyfour = True      
# Check to see that the string 'whole' is not in the value of the key window. If it's not, then assign to the variable whole the value of True, otherwise False.
whole = False
for items in nested['window']:
    if 'whole' not in items:
        whole = False
    else:
        whole = True
...
for key in nested_d.keys():
    if key == 'London':
        london_gold += nested_d[key]['Great Britain']

#6) Below, we have provided a nested dictionary. Index into the dictionary to create variables that we have listed in the 
# ActiveCode window. ???????????????????????????????????
...
nested_d = {'Beijing':{'China':51, 'USA':36, 'Russia':22, 'Great Britain':19}, 'London':{'USA':46, 'China':38, 'Great Britain':29, 'Russia':22}, 'Rio':{'USA':35, 'Great Britain':22, 'China':20, 'Germany':13}}
US_count = []
for olimpics in nested_d.keys():
    US_count.append(nested_d[olimpics]['USA'])
