...
#2)Write one for loop to print out each element of the list several_things. Then, write another for loop to print out the TYPE of each element of the list several_things. 
...
#5) addition_str is a string with a list of numbers separated by the + sign. Write code that uses the accumulation pattern to take the sum of all
# of the numbers and assigns it to sum_val (an integer). (You should use the .split("+") function to split by "+" and int() to cast to an integer).
#addition_str = "2+5+10+20"
#sum_list = addition_str.split("+")
#sum_val = 0
#for str_number in sum_list:
#    sum_val += int(str_number)
...
#(sum divided by number of items) and assigns it to avg_temp. Do not hard code your answer (i.e., make your code compute both the sum or the number of items 
...
#num_words_list = list(range(len(words)))
#count = 0
#for word in words:
#    num_words_list[count] = len(word)
#    count+=1
...
#10) Write a program that uses the turtle module and a for loop to draw something. It doesn’t have to be complicated, but draw something different than 
# we have done in the past. (Hint: if you are drawing something complicated, it could get tedious to watch it draw over and over. Try setting .speed(10) 
# for the turtle to draw fast, or .speed(0) for it to draw super fast with no animation.)
#import turtle
#wn = turtle.Screen()
#jazz = turtle.Turtle()
#for i in range(5):
